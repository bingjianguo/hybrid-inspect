    ...
    */
    ...
    co(fetchReqData)
      ...
      .then(co.wrap(function*() {
        let userModifiedInfo = _.cloneDeep(requestDetail);
        const userRuleArray = userRule;
        for (let iIndex = 0; iIndex < userRuleArray.length; iIndex++) {
          const item = userRuleArray[iIndex];
          const temp = (yield item.beforeSendRequest(_.assign({}, userModifiedInfo))) || {};
      ...
      .then(co.wrap(function*(responseData) {
        if (responseData._directlyPassToRespond) {
          ...
          // TODO: err etimeout
          let userModifiedResponseData = responseData;
          const userRuleArray = userRule;
          for (let iIndex = 0; iIndex < userRuleArray.length; iIndex++) {
            const userRuleItem = userRuleArray[iIndex];
            const temp = yield userRuleItem.beforeSendResponse(
              Object.assign({}, requestDetail),
              Object.assign({}, userModifiedResponseData)
            );
          ...
          return userModifiedResponseData || responseData;
        }
      }))
      ...
      .catch(co.wrap(function*(error) {
        ...
        try {
          ...
          for (let iIndex = 0; iIndex < userRuleArray.length; iIndex++) {
            const item = userRuleArray[iIndex];
            const tempUserResponse = yield item.onError(Object.assign({}, requestDetail), error);
    ...
    */
    co(function *() {
      ...
      if (reqHandlerCtx.forceProxyHttps) {
        ...
        for (let iIndex = 0; iIndex < userRuleArray.length; iIndex++) {
          const item = userRuleArray[iIndex];
          shouldIntercept = shouldIntercept || (yield item.beforeDealHttpsRequest(requestDetail));
        }
    ...
    .catch(co.wrap(function *(error) {
      ...
      try {
        ...
        for (let iIndex = 0; iIndex < userRuleArray.length; iIndex++) {
          const item = userRuleArray[iIndex];
          yield item.onConnectError(requestDetail, error);
        }
...
class RequestHandler {
  ...
  constructor(config, rule, recorder) {
    ...
    const userRule = ruleArray.map(item => _.assign({}, default_rule, item));

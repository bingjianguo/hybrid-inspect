	...
	export class DOMTimelineClient extends ClientPlugin {
        ...
        public startClientSide(): void {
            ...
            domTimelineOptions.considerLoggingRecords = function(c,entries,s) {
                if(!domData) {
                   // 以下打印语句会造成页面级别卡死，具体情况是指，当timeline未启用时，该打印语句一直会
                   // 在console中重复出现
                   if (!self._bInfoNotReady) {
                       self._bInfoNotReady = true;
                       console.warn("domData not yet initalized; entries ignored: ", entries);
                   }
                   return;
